...
export interface GrvtMakerEngineSnapshot {
  ready: boolean;
  symbol: string;
  topBid: number | null;
  topAsk: number | null;
...
type GrvtMakerEvent = "update";
type GrvtMakerListener = (snapshot: GrvtMakerEngineSnapshot) => void;
...
export class GrvtMakerEngine {
  ...
  private readonly events = new StrategyEventEmitter<GrvtMakerEvent, GrvtMakerEngineSnapshot>();
  ...
  on(event: GrvtMakerEvent, handler: GrvtMakerListener): void {
    this.events.on(event, handler);
  }

  off(event: GrvtMakerEvent, handler: GrvtMakerListener): void {
    this.events.off(event, handler);
  }

  getSnapshot(): GrvtMakerEngineSnapshot {
  ...
  private async tick(): Promise<void> {
    ...
    try {
      ...
      this.sessionVolume.update(position, this.getReferencePrice());
      await this.syncOrders(desired, position);
      await this.checkRisk(position, Number(closeBidPrice), Number(closeAskPrice));
      this.emitUpdate();
    } catch (error) {
      if (isRateLimitError(error)) {
        hadRateLimit = true;
        this.rateLimit.registerRateLimit("grvt-maker");
        await this.enforceRateLimitStop();
        this.tradeLog.push("warn", `GrvtMakerEngine 429: ${String(error)}`);
      } else {
        this.tradeLog.push("error", `GRVT做市循环异常: ${String(error)}`);
      }
      this.emitUpdate();
    } finally {
  ...
  private async handlePositionChange(position: PositionSnapshot, prevAbsPosition: number = Math.abs(this.lastPositionAmt)): Promise<void> {
    ...
    if (absPosition > EPS) {
      ...
      const entryOrdersToCancel = this.openOrders.filter((o) => {
        const isEntry = !o.reduceOnly;
        const isOpposite = o.side === oppositeSide;
        const isNotStop = String(o.type).toUpperCase() !== "STOP_MARKET" && 
                          (!Number.isFinite(Number(o.stopPrice)) || Number(o.stopPrice) === 0);
        return isEntry && isOpposite && isNotStop;
      });
  ...
  private async syncOrders(targets: DesiredOrder[], position: PositionSnapshot): Promise<void> {
    ...
    const regularOrders = openOrders.filter((o) => {
      const hasStopPrice = Number.isFinite(Number(o.stopPrice)) && Number(o.stopPrice) > 0;
      return String(o.type).toUpperCase() !== "STOP_MARKET" && !hasStopPrice;
    });
    ...
    for (const target of targets) {
      const matching = regularOrders.find((o) => 
        o.side === target.side && 
        Math.abs(Number(o.price) - Number(target.price)) < this.config.priceTick / 2 &&
        o.reduceOnly === target.reduceOnly
      );
    ...
    // Cancel orders that don't match any target
    for (const order of regularOrders) {
      const matching = targets.find((t) => 
        t.side === order.side && 
        Math.abs(Number(order.price) - Number(t.price)) < this.config.priceTick / 2 &&
        order.reduceOnly === t.reduceOnly
      );
  ...
  private async ensureStopLossOrder(position: PositionSnapshot, lastPrice: number | null): Promise<void> {
    ...
    if (needPlace) {
      try {
        ...
        this.tradeLog.push("stop", `已添加止损单: ${stopSide} STOP @ ${formatPriceToString(roundedTarget, Math.max(0, Math.floor(Math.log10(1 / this.config.priceTick))))}`);
      } catch (err) {
  ...
  private buildSnapshot(): GrvtMakerEngineSnapshot {
    const position = getPosition(this.accountSnapshot, this.config.symbol);
    const { topBid, topAsk } = getTopPrices(this.depthSnapshot);
